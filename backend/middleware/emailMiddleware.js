...
const sendSubUserTempPass = async (subUser, generatedPass, req, res) => {
    ...
    transporter.sendMail(options, function (err, info) {
            ...
            res.status(200).json({"firstName": subUser.firstName, "lastName": subUser.lastName, "email": subUser.email, "OTPStatus": info})   
          } )
    } catch (error) {
        res.status(400).json(error)
    }
